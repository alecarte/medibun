...
| S3     | Built — in review | HealthcareService+SchedulingParameters builders + $find/$book wrappers (wire shapes source-verified, unit-tested); services catalog migration + repo; categorical color tokens (AA-tested); `pnpm demo:seed` with $find self-check (live run needs a Docker-capable machine); DATA_MODEL.md A2 amendment recorded. |
