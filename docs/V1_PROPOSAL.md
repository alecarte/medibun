...
| R1     | Merged (PR #21)         | 2026-08-13, merged by Alec — the B2 schema walkthrough; five staging tables + 4D adapter + local import CLI on `main`. Real-export verification waits on R0 (§7 local runbook). Follow-up (small, not R1): `scripts/seed-demo.ts` prints a raw caught `err.message` — synthetic-only today, but both scripts should share the import CLI's failure-printing rule. |
