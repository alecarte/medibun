...
| R2     | **Built, at the B2 gate** (2026-08-15) | Five stacked PRs: **R2a** (#24) — the `service_categories` + `staged_transactions` migration, **pending Alec's schema walkthrough and merge** (the B2 gate's own rule); **R2b** (#25) — the 4D report-layout pre-pass and the real column maps; **R2c** (#26) — segmentation (dormant, revenue-driven; unconverted consults, degraded per R0), the category seeder, and the Leak Report generator (self-contained print-quality HTML, aggregates only, B8 quoted verbatim); **#27** — review follow-ups (one error-scrub rule, one visit-grouping definition, one script bootstrap, single-pass indexes), zero behavior change; **#28** — a second review round's correctness fixes: readers take **only the latest counted import run** per entity (superseded rows were inflating tickets and the headline), the dormancy exclusion compares **practice-local calendar days** (it under-excluded east of UTC, contacting patients who had already rebooked), the classifier no longer swallows lone-cell rows, and money has one presentation policy. Four `security-reviewer` passes, every finding remediated; 415 tests. Verified on synthetic report-layout fixtures ONLY. The slice's own verify step — **the Handal Leak Report from real data plus Alec's face-validity sign-off** — is **owed**, and runs via the RECOVERY_DESIGN.md §7 local runbook once the stack merges. Two operator preconditions recorded there: the **blocking `*_raw` allow-list verdict** before the first non-synthetic import, and **one full-range export file per entity** (split files are unsupported, and the import CLI warns). |
| R3–R6  | Not started                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
