...
| R2     | **Built, at the B2 gate** (2026-08-14) | Three stacked PRs: **R2a** — the `service_categories` + `staged_transactions` migration, **pending Alec's schema walkthrough and merge** (the B2 gate's own rule); **R2b** — the 4D report-layout pre-pass and the real column maps; **R2c** — segmentation (dormant, revenue-driven; unconverted consults, degraded per R0), the category seeder, and the Leak Report generator (self-contained print-quality HTML, aggregates only, B8 quoted verbatim). Verified end to end on synthetic report-layout fixtures. The slice's own verify step — **the Handal Leak Report from real data plus Alec's face-validity sign-off** — is **owed**, and runs via the RECOVERY_DESIGN.md §7 local runbook once R2a merges. Security-review remediations landed with R2a: the writer posture, and the **blocking `*_raw` allow-list precondition on the first non-synthetic import**. |
| R3–R6  | Not started                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
