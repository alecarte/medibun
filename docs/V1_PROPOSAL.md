...
| Slice  | Status                                     | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------ | ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R0     | **Complete** (2026-08-14)                  | Verify step met: field mapping recorded (RECOVERY_DESIGN review log, 2026-08-14, three entries); pools marked — **dormant feasible** (revenue-driven), **unconverted consults feasible (degraded**: quote-created, surgical, name-only join**)**, **inquiries infeasible at Handal** (4D doesn't track them) — R2 segmentation targets the two feasible pools. CSVs are report-layout dumps → the R1 adapter gains a normalization pre-pass; real-data R1 verification (counts vs in-file 4D totals) runs at first local import once that lands.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| R1     | **Merged** (PR #21, Alec, 2026-08-13)      | Staging migration (B2's first — approved by the merge, per the gate's own rule), `SourceAdapter` + 4D CSV adapter, local import CLI with rejects report. Synthetic fixtures only; real-export verification waits on R0 and runs via the §7 local runbook. Follow-up **done** (2026-08-15, R2 review round 2): `scripts/seed-demo.ts` no longer prints a raw caught `err.message` — it runs through `runLocalScript` with its own `makeErrorLine` safe classes, so every local script now shares one failure-printing rule.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| R2     | **Merged** (PRs #24–#29, Alec, 2026-08-27) | **B2 closed: approved as built** — `service_categories` + `staged_transactions` (kept; it carries the dormancy signal, the revenue-average ticket math, and the latest-import filter) + `phone_type`, with the empty-table/truncate assumption and the relaxed appointment join-key validation accepted at the walkthrough. Shipped: the 4D report-layout pre-pass and real column maps (#25); segmentation — dormant (revenue-driven) and unconverted consults (degraded per R0) — the category seeder, and the aggregates-only print-quality Leak Report with B8 quoted verbatim (#26); consolidation (#27); a second review round's correctness fixes (#28) — latest-counted-import reads, practice-local dormancy days, classifier precedence, one money policy; integration (#29). Four `security-reviewer` passes, every finding remediated; 415 tests. **Verified on synthetic fixtures ONLY — the slice's own verify step is OWED**: the Handal Leak Report from real data plus Alec's face-validity sign-off, via the RECOVERY_DESIGN.md §7 local runbook. Two operator preconditions before that run: the **blocking `*_raw` allow-list verdict**, and **one full-range export file per entity** (split files are unsupported; the import CLI warns). |
| R3–R6  | Not started                                |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- 2026-08-27 — **B2 CLOSED (Alec, in-session): approved as built.** The R2a walkthrough answered: `staged_transactions` **kept** (striking it would have unwound R2c — dormancy falls back to the appointment export, which R0 confirmed carries no status column), `phone_type` **kept**, and the two disclosures accepted explicitly — the migration's empty-table assumption (a populated local `staged_consults` is **truncated**, not migrated around) and R1's deliberately relaxed appointment join-key validation. The R2 stack (#24–#29) merged the same day. **R3 ∥ R4 are now unblocked** (§5: they may be built in parallel branches after R2). B2 has no further migrations pending until R5's attribution ledger.
- 2026-08-13 — **B-series closeout (Alec, in-session)**: **B4 decided** — Twilio for SMS
  ...
  a BAA). **B3 signed** — the messaging standard rendered into CLAUDE.md (RECOVERY_DESIGN §5 remains
  the full standard — token properties and STOP language live there). **B6
